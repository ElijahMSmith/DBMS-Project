...
router.post('/membership', async (req, res) => {
    ...
    return sql.connect(config).then(async (pool) => {
        ...
        if(result.recordset.length > 0)
            return res.status(406).send({"error": "A relationship between that student and RSO already exists"});
        ...
        // Otherwise...
        return res.status(500).send({"error": "An unknown internal server error occured."});
    }).catch((err) => {
        return res.status(500).send({"error": err})
    });
});
...
router.delete('/membership', async (req, res) => {
    ...
    return sql.connect(config).then(async (pool) => {
        ...
        // If there is already a relationship... do nothing
        if(result.recordset.length <= 0)
            return res.status(406).send({"error": "A relationship between that student and RSO does not exist"});
        ...
        // Check that a change was made
        if(result.rowsAffected > 0)
            return res.status(200).send({"message": "Relationship Deleted"});
