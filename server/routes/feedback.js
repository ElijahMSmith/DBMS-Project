    ...
    return sql.connect(config).then(async(pool) => {
        ...
        if(uid && eid && description) {
            ...
            if(result.rowsAffected <= 0)
                return res.status(500).send({"error": "Failed to create a comment with that information"});
        ...
        if(cid && description) {
            ...
            const request = `UPDATE Comments SET description='${description}' WHERE eid='${cid}'`;
            const result = await pool.query(request);
            if(result.rowsAffected <= 0)
                return res.status(406).send({"error": "Failed to update a comment with that CID"});
...
router.post('/rating', async(req, res) => {
    ...
    return sql.connect(config).then(async(pool) => {
        ...
        if((await pool.query(request)).recordset.length > 0) {
            ...
            if(result.rowsAffected <= 0)
                return res.status(406).send({"error": "Failed to update a rating with that UID and EID"});
            ...
            if(result.rowsAffected <= 0)
                return res.status(406).send({"error": "Failed to create a rating with that UID and EID"});
            
            return res.status(200).send({"message": "Rating created successfully"});
        }
    })
});
