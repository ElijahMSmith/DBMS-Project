...
router.post('/', async (req, res) => {
    let { unid, name, description, numStudents } = req.body;
        ...
        .then(async (pool) => {
            ...
            if (result.rowsAffected <= 0)
                return res.status(406).send({
                    error: 'No Changes Made. University ID might not be found, or, some issue with the input data',
                });
    ...
    return connectionPromise
        .then(async (pool) => {
            ...
            if (unid == undefined) {
                ...
                return res
                    .status(200)
                    .send({ universities: response.recordset });
            }
            ...
            if (response.recordset.length <= 0)
                return res
                    .status(404)
                    .send({ error: 'No University Found with that UNID' });
    ...
    return connectionPromise
        .then(async (pool) => {
            ...
            if (result.recordset.length > 0)
                return res.status(406).send({
                    error: 'An image with that URL already exists for that university',
                });
            ...
            if (result.rowsAffected > 0)
                return res
                    .status(200)
                    .send({ message: 'URL added successfully' });
        ...
        .then(async (pool) => {
            ...
            if (result.recordset.length == 0)
                return res.status(404).send({
                    error: 'An image with that URL already does not exist for that university',
                });
            ...
            if (result.rowsAffected > 0)
                return res
                    .status(200)
                    .send({ message: 'URL removed successfully' });
